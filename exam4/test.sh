...
test_line () {
	...
	sleep .250
	echo >> out.res
	leaks a.out > leaks.res 2> /dev/null
	if grep "ROOT LEAK" < leaks.res > /dev/null 2> /dev/null ; then
		printf "\e[0;31mLEAKS\n\e[0m"
	fi
	pid=$( pgrep microshell )
	printf "\e[0;31m"
	lsof -c a.out | grep $pid | grep -v cwd | grep -v txt | grep -v 0r | grep -v 1w | grep -v 2u | grep a.out
	printf "\e[0m"
	kill -9 $pid
	wait $pid 2>/dev/null
	#cat -e out.res > out
...
gcc -g -Wall -Werror -Wextra -DTEST_SH microshell.c utils.c -o a.out
printf "\e[1;36mTest\n\e[0m"
